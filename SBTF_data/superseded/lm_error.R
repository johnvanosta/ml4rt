...
ggplot(predict_df, aes(x = log(mean_distance_from_tower))) +
  geom_histogram(binwidth = 0.5, fill = "blue", color = "black", alpha = 0.7) +
  labs(title = "Histogram of Mean Distance from Tower",
       x = "Mean Distance from Tower",
       y = "Frequency") +
  theme_minimal()

# Create facet wrap scatter plots by training data type
ggplot(predict_df, aes(x = log(mean_distance_from_tower), y = log_error_m)) +
  geom_point(alpha = 0.7) +
  labs(title = "Scatter Plots of Mean Distance from Tower vs. Log Error",
       x = "Mean Distance from Tower",
       y = "Log Error (m)") +
  theme_minimal() +
  facet_wrap(~ training_data_type, ncol = 2)

#predict_df$inteval_seconds <- as.factor(predict_df$inteval_seconds)
model1 <- lm(log_error_m ~ Data_type * training_data_type + Tower_count +
               inteval_seconds + log_mean_distance_from_tower * training_data_type, 
             data = predict_df)
...
model2 <- lm(log_error_m ~ Data_type * training_data_type * log_mean_distance_from_tower + Tower_count +
               inteval_seconds, 
             data = predict_df)
summary(model2)
anova(model1, model2)

model3 <- lm(log_error_m ~ Data_type * training_data_type + Tower_count +
...
ggsave("Paper_results/Figures/positional_error_covariates_20230813.png", plot = plot_grid,
